...
def precipitation():
    ...
    results= session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >= prev_date).order_by(Measurement.date.desc()).all()


    prec = dict(results)

    print(f"Results for Precipitation - {prec}")
